...
#ifndef DUNE_COMMON_STD_LAYOUT_RIGHT_PADDED_HH
#define DUNE_COMMON_STD_LAYOUT_RIGHT_PADDED_HH
...
struct IsLayoutRightPadded<layout_right_padded<PaddingValue>> : std::true_type {};

template <class Layout>
inline constexpr bool is_layout_right_padded_v = IsLayoutRightPadded<Layout>::value;

} // end namespace Impl

/**
 * \brief A right layout mapping with optional padding of the next-to-last stride.
 * \ingroup CxxUtilities
 *
 * This layout follows the `layout_right_padded` mapping from
...
public:
  ...
  constexpr mapping (const extents_type& e) noexcept
    : extents_(e)
    , stride_before_last_(default_stride_before_last(e))
  {}
  ...
  constexpr mapping (const layout_stride::mapping<OtherExtents>& m) noexcept
    : extents_(m.extents())
    , stride_before_last_(Extents::rank() > 1 ? m.stride(Extents::rank()-2) : index_type(0))
  {
#ifndef NDEBUG
    if constexpr(Extents::rank() > 0)
      assert(m.stride(Extents::rank()-1) == 1);
    if constexpr(Extents::rank() > 1)
      for (rank_type r = 0; r + 2 < Extents::rank(); ++r)
        assert(m.stride(r) == stride_before_last_ * rev_product(r));
#endif
  }

  template <class OtherMapping,
    std::enable_if_t<Impl::is_layout_right_padded_v<typename OtherMapping::layout_type>, int> = 0,
  ...
  {
    return Extents::rank() <= 1 || stride_before_last_ == extents_.extent(Extents::rank()-1);
  }
  ...
  constexpr index_type stride (rank_type r) const noexcept
  {
    ...
    if (r + 1 == Extents::rank())
      return 1;
    if (r + 2 == Extents::rank())
      return stride_before_last_;
    return stride_before_last_ * rev_product(r);
  }
...
private:
  ...
  static constexpr index_type default_stride_before_last (const extents_type& e) noexcept
  {
    if constexpr(Extents::rank() <= 1)
      ...
      return e.extent(Extents::rank()-1);
    else
      return padded_stride(index_type(PaddingValue), e.extent(Extents::rank()-1));
  }

  constexpr index_type rev_product (rank_type r) const noexcept
  {
    index_type prod = 1;
    for (rank_type k = r + 1; k + 1 < Extents::rank(); ++k)
...
private:
  ...
  index_type stride_before_last_ = 0;
};

} // end namespace Dune::Std

#endif // DUNE_COMMON_STD_LAYOUT_RIGHT_PADDED_HH
